...
@component('admin::include.page.header')
    @slot('title', clean(trans('ebook::reports.pre_book_visitor')))
    <li class="nav-item">{{ clean(trans('ebook::reports.reports')) }}</li>
    <li class="separator">
        <i class="flaticon-right-arrow"></i>
    </li>
    <li class="nav-item">{{ clean(trans('ebook::reports.pre_book_visitor')) }}</li>
@endcomponent
...
<div class="card">
    <div class="card-header">
        <div class="card-head-row card-tools-still-right">
            <h4 class="card-title">{{ clean(trans("ebook::reports.pre_book_visitor")) }}</h4>
            <div class="card-tools">
                <select id="filter-user" class="form-control" >
                    <option value="">Select a User</option>
                    @foreach ($users as $uid=>$user)
                        @if($sUser == $uid)
                            <option   selected value="{{ $uid }}"> {{ $user }}</option>
                        @else
...
<script>
    (function () {
        ...
        $(document).on("change","#filter-user",function(e){
            e.preventDefault();
            var url="{{ route('admin.reports.pre_book_visitor') }}";
            if($(this).val()!=''){
